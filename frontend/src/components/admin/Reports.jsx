...
//             className="flex items-center space-x-2 bg-white border border-gray-300 px-4 py-2 rounded-lg text-gray-700 hover:bg-gray-50 print:hidden">
...
// Remove Navbar import if it's already in App.jsx to stop the double header
import authService from '../../services/auth';
import api from '../../services/api';
import { 
  Chart as ChartJS, 
...
const Reports = () => {
  ...
  // 1. Create a reference for the area we want to capture
  ...
  const getCategoryDoughnutData = () => {
    ...
    return {
      ...
      datasets: [{
        data: speciesDist.byCategory.map(d => d.count),
        backgroundColor: ['#16a34a', '#2563eb', '#db2777', '#9333ea', '#ea580c', '#0891b2', '#ca8a04'],
        borderWidth: 1,
      }],
    };
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Header - Stays outside the Ref so the download button isn't inside the PDF */}
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">System Reports</h1>
            <p className="text-gray-600">Analytics and trends overview</p>
        ...
        {/* 3. ATTACH THE REF HERE TO CAPTURE EVERYTHING BELOW */}
        <div ref={reportRef} className="space-y-8 p-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
              <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center">
                <Calendar className="h-5 w-5 mr-2 text-green-600" /> Sighting Activity
              </h3>
              <div className="h-64">
                <Line options={{ maintainAspectRatio: false, plugins: { legend: { display: false } } }} data={getSightingLineData()} />
              </div>
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
              <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center">
                <FileText className="h-5 w-5 mr-2 text-red-600" /> Incidents by Type
              </h3>
              <div className="h-64">
                <Bar options={{ maintainAspectRatio: false }} data={getIncidentBarData()} />
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {sightingTrends?.activeObservers?.slice(0, 4).map((obs) => (
                <div key={obs.id} className="flex items-center p-4 bg-gray-50 rounded-lg">
                  ...
                  <div className="ml-4">
                    <p className="text-sm font-medium text-gray-900">{obs.firstName} {obs.lastName}</p>
                    <p className="text-xs text-gray-500">{obs.sightingCount} reports</p>
                  </div>
                </div>
              ))}
            </div>
