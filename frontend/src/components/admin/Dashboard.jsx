  ...
  const StatCard = ({ icon: Icon, title, value }) => (
    <div className="border border-ops-line bg-ops-surface p-5">
      <div className="flex items-center justify-between mb-3">
        <p className="font-mono text-[10px] uppercase tracking-widest text-steel/50">{title}</p>
        <Icon className="h-4 w-4 text-cyan" />
      </div>
      <p className="font-display text-3xl font-semibold text-steel">{value || 0}</p>
    </div>
  );
      ...
      <div className="min-h-screen bg-ops text-steel font-body">
        ...
        <div className="flex items-center justify-center h-screen">
          <div className="text-center">
            ...
            <p className="mt-4 font-mono text-xs uppercase tracking-widest text-steel/50">Loading dashboard...</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-wrap gap-3 mb-8">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 border border-dashed border-ops-line hover:border-cyan hover:bg-ops-surface transition-colors font-mono text-[11px] uppercase tracking-widest text-steel/70"
          >
            <Users className="h-3.5 w-3.5 text-steel/40" />
            Manage Users
          </button>
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 border border-dashed border-ops-line hover:border-cyan hover:bg-ops-surface transition-colors font-mono text-[11px] uppercase tracking-widest text-steel/70"
          >
            <Layers className="h-3.5 w-3.5 text-steel/40" />
            Manage Species
          </button>
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 border border-dashed border-ops-line hover:border-cyan hover:bg-ops-surface transition-colors font-mono text-[11px] uppercase tracking-widest text-steel/70"
          >
            <TrendingUp className="h-3.5 w-3.5 text-steel/40" />
            View Reports
          </button>
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 border border-dashed border-ops-line hover:border-cyan hover:bg-ops-surface transition-colors font-mono text-[11px] uppercase tracking-widest text-steel/70"
          >
            <Radio className="h-3.5 w-3.5 text-steel/40" />
            IoT Monitor
          </button>
        </div>

        {/* Tabs — now the primary content, with pending counts surfaced */}
        <div className="border border-ops-line bg-ops-surface mb-6">
          <div className="border-b border-ops-line">
              ...
              <button
                ...
                className={`py-4 px-1 border-b-2 font-mono text-xs uppercase tracking-widest transition-colors ${
                  activeTab === 'overview'
                    ...
                    : 'border-transparent text-steel/40 hover:text-steel/70'
                }`}
              >
                Overview
              </button>
          ...
          <div className="p-6">
            {activeTab === 'overview' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="border border-ops-line p-5">
                  <h3 className="font-mono text-[10px] uppercase tracking-widest text-steel/50 mb-2">
                    Endangered Species
                  </h3>
                  <p className="font-display text-3xl font-semibold text-signal">
                    {stats?.endangeredSpecies || 0}
                  </p>
                  <p className="text-xs text-steel/50 mt-2">Requiring special attention</p>
                </div>
                <div className="border border-ops-line p-5">
                  <h3 className="font-mono text-[10px] uppercase tracking-widest text-steel/50 mb-2">
                    Active IoT Sensors
                  ...
                  <p className="text-xs text-steel/50 mt-2">Transmitting real-time data</p>
                </div>
              </div>
            )}
                        ...
                        <div className="flex items-center gap-3 flex-shrink-0">
                          ...
                          {!sighting.verified && (
                            <button
                              ...
                              className="flex items-center gap-1.5 border border-amber text-amber font-mono text-[10px] uppercase tracking-widest px-3 py-1.5 hover:bg-amber hover:text-ops transition-colors disabled:opacity-50"
                            >
              ...
              <div className="border border-ops-line">
                {recentIncidents.length === 0 ? (
                  ...
                  recentIncidents.map((incident) => (
                    <div key={incident.id} className="field-tag border-l-2 border-l-signal">
                      <div className="flex-1 flex justify-between items-start gap-4">
                        <div>
                          <h4 className="font-display font-semibold text-sm">
                            {incident.incidentType}
                          </h4>
                          <p className="text-sm text-steel/60 mt-1">
                        ...
                        <div className="text-right flex-shrink-0">
                          ...
                          <p className="text-xs text-steel/40 mt-2">{incident.status}</p>
                        </div>
                      </div>
                    </div>
                  ))
        ...
        <div className="border border-ops-line bg-ops-surface">
          <button
            onClick={() => setInsightsOpen(prev => !prev)}
            className="w-full flex items-center justify-between p-6"
          >
            <div className="flex items-center">
              <ShieldAlert className="h-4 w-4 text-cyan mr-2" />
              <h2 className="font-display text-base font-semibold">Insights</h2>
              {insightCount > 0 && (
                <span className="ml-2 font-mono text-[11px] px-2 py-0.5 border border-cyan-dim text-cyan">
                  {insightCount}
                </span>
              )}
            </div>
            <ChevronDown className={`h-4 w-4 text-steel/40 transition-transform ${insightsOpen ? 'rotate-180' : ''}`} />
          </button>
            ...
            <div className="px-6 pb-6 space-y-8">
              ...
              <div>
                <div className="flex items-center mb-4">
                  ...
                  <span className="ml-2 font-mono text-[11px] text-steel/40">ML-ranked, pending sightings needing closer review</span>
                </div>

                {verificationError ? (
                  <p className="text-center font-mono text-xs uppercase tracking-widest text-signal py-8">{verificationError}</p>
                ) : verificationPriority.length === 0 ? (
                  <p className="text-center font-mono text-xs uppercase tracking-widest text-steel/40 py-8">No pending sightings</p>
                ) : (
                  <div className="border border-ops-line">
                        ...
                        <div className="flex-1 flex justify-between items-center gap-4">
                          ...
                          <div className="flex items-center gap-3 flex-shrink-0">
                            <span className={`font-mono text-[10px] uppercase tracking-widest px-2 py-1 border ${
                              v.priorityLevel === 'Critical' ? 'border-signal text-signal' :
                              v.priorityLevel === 'High' ? 'border-cyan text-cyan' :
                              v.priorityLevel === 'Medium' ? 'border-cyan-dim text-cyan' :
                              'border-amber text-amber'
                            }`}>
              ...
              <div>
                <div className="flex items-center mb-4">
                  <UserCog className="h-4 w-4 text-amber mr-2" />
                  <h3 className="font-display text-sm font-semibold">User Activity Anomalies</h3>
                  <span className="ml-2 font-mono text-[11px] text-steel/40">Per-user z-score vs their own 8-week baseline</span>
                </div>

                {userAnomalyError ? (
                  <p className="text-center font-mono text-xs uppercase tracking-widest text-signal py-8">{userAnomalyError}</p>
                ) : anomalyCount === 0 ? (
                  <p className="text-center font-mono text-xs uppercase tracking-widest text-steel/40 py-8">No unusual activity detected</p>
                ) : (
                  <div className="border border-ops-line">
                      ...
                      <div key={u.userId} className="field-tag">
                        <div className="flex-1 flex justify-between items-center gap-4">
                          ...
                          <span className={`font-mono text-[10px] uppercase tracking-widest px-2 py-1 border flex-shrink-0 ${
                            u.direction === 'spike' ? 'border-cyan text-cyan' : 'border-amber text-amber'
                          }`}>
